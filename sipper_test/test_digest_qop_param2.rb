# test that has qop parameter in the challenge and hence cnonce is present.

require 'driven_sip_test_case'

class TestDigestQopParam2 < DrivenSipTestCase
  ...
  def setup
    ...
    module SipInline
      class UasDigestS6Controller < SIP::SipTestDriverController
          ...
          if result
            ...
            if old
              ...
              r = session.create_challenge_response(session.irequest, false,
                "sipper2.com", nil, false, false, true)          
              session.send(r)
            end  
          end
        end
        ...
        def on_success_res(s)
          s.invalidate(true)
          s.flow_completed_for("TestDigestQopParam2")
        end
      ...
      class UacDigestS6Controller < SIP::SipTestDriverController
    ...
    set_controller("SipInline::UacDigestS6Controller")
  end
  
  
  def test_digest_controllers
    self.expected_flow = ["> INVITE", "< 100", "< 401",  "> ACK", "! false", "> INVITE", "< 100", "< 200", "> ACK", "< BYE", "> 200"]
    start_controller
    verify_call_flow(:out)
    self.expected_flow = ["< INVITE", "> 100", "> 401", "< ACK", "< INVITE", "> 100", "! false", "> 200", "< ACK", "> BYE", "< 200"]
    verify_call_flow(:in)
  end
end