...
class TestTargetRefreshProxyUdp < DrivenSipTestCase
  ...
  def setup
    ...
    module SipInline
      class UasRt4Controller < SIP::SipTestDriverController
      ...
      class UacRt4Controller < SIP::SipTestDriverController
        ...
        def on_success_res_for_bye(session)
          session.invalidate(true)
          session.flow_completed_for("TestTargetRefreshProxyUdp")
        end
    ...
    set_controller("SipInline::UacRt4Controller")
  end
  ...
  def test_rt1_controllers
    ...
    self.expected_flow = ["< INVITE", "> 100", "! double_via", "> 200", "< ACK", "< BYE", "! single_via", "> 200"]
    verify_call_flow(:in)
  end
