...
class TestTargetRefreshWithRrUpdateProxy < DrivenSipTestCase
  ...
  def setup
    ...
    unless RUBY_PLATFORM =~ /mswin/
      system("srun -p #{SipperConfigurator[:LocalTestPort][1]+2} -o #{SipperConfigurator[:LocalTestPort][1]} -rf 3 -c #{File.join(SipperConfigurator[:SipperBasePath],"sipper_test", "rr_proxy.rb")} &")    
    end
    ...
    module SipInline
      class UasRt8Controller < SIP::SipTestDriverController
      ...
      class UacRt8Controller < SIP::SipTestDriverController
        ...
        def start
          ...
          #r.route = "sip:#{SipperConfigurator[:LocalSipperIP]}:#{SipperConfigurator[:LocalTestPort][1]+2};lr"    
          u.send(r)
        ...
        def on_success_res_for_bye(session)
          session.invalidate(true)
          session.flow_completed_for("TestTargetRefreshWithRrUpdateProxy")
        end
    ...
    set_controller("SipInline::UacRt8Controller")
  end
  ...
  def test_rt1_controllers
    ...
    self.expected_flow = ["< INVITE", "> 100", "! double_via", "> 200", "< ACK", "> UPDATE", "< 200",  "< BYE", "! double_via", "! nkhan", "> 200"]
    verify_call_flow(:in)
  end
